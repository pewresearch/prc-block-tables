...
class Generate_Tabular_Data {
	...
	public function register_ability() {
		wp_register_ability(
			self::$ability_name,
			array(
				'label'               => __( 'Generate Tabular Data', 'prc-block-tables' ),
				'description'         => __( 'Generates tabular data in markdown format based on a prompt.', 'prc-block-tables' ),
...
- Use standard markdown table syntax with pipes and dashes.';
	...
	public function execute( $input ) {
		...
		if (
			! function_exists( 'PRC\Platform\AI\Utils\refine_search_term' )
			|| ! function_exists( 'PRC\Platform\AI\Utils\refine_search_term_to_list_of_topics' )
		) {
			return array(
				'error' => 'AI utilities are not available.',
				'table' => '',
			);
		}
		...
		if ( is_wp_error( $table ) ) {
			return array(
				'error' => 'AI generation failed: ' . $table->get_error_message(),
				'table' => '',
			);
		}
