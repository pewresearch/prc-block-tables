...
7. Renders via the shared `\PRC\Platform\Blocks\render_tabs()` helper from `@prc/block-library`.
...
-   `prc-block/tabs` -- Tab container for displaying pivoted data groups (from `@prc/block-library`)
-   `prc-block/tab` -- Individual tab within the tabs container (from `@prc/block-library`)
