/* eslint-disable max-lines -- Deprecations accumulate full save() copies by design */
/**
...
// ---------------------------------------------------------------------------
// v3 — resize/reorder save shape with <colgroup>, but cell hover color still
//      serialized as a literal `hover-background-color` property (browsers
//      ignore; KSES strips). Current save recasts to `--hover-background-color`.
// ---------------------------------------------------------------------------

function getVColStartForCellV3(row: Row, cellIndex: number): number {
	let v = 0;
...
function getSavedColumnCountV3(head: Row[], body: Row[], foot: Row[]): number {
...
const v3 = {
	...
	save({ attributes }: BlockSaveProps<BlockAttributes>) {
		...
		const columnCount = getSavedColumnCountV3(head, body, foot);
						...
						<tr key={rowIndex}>
							{cells.map(
								(
									...
									const vColStart = getVColStartForCellV3(
										row,
										cellIndex
									);
